<#
Shared core for the Lionheart mod installer and the mod manager UI.

Dot-sourced by both front ends -- mod-installer.ps1 (command line, driven by Install.bat)
and ModManager.ps1 (a window). Everything that knows about the game, the archive format or
the mod format lives here, so the two cannot drift: a second copy of the delta applier or
the ZIP writer is exactly the kind of duplication that ends with one of them being subtly
wrong.

WHAT INSTALLING DOES
--------------------
Rebuilds the player's data.dat with the mod applied: reads their archive, replaces the
entries the mod changes, adds the ones it introduces, writes a new archive in its place.
That leaves the install in the configuration the game actually ships in.

Installing by copying into a loose <game>\data\ directory also works -- the engine reads
loose files in preference to the archive -- but a stock install has no such directory;
GOG's manifest lists 16 files and zero directories. Where one does exist, the mod's files
are written there too, or the stale loose copies would shadow the rebuild and the install
would silently do nothing.

WHAT A RELEASE CONTAINS
-----------------------
No content from the shipped game. Files that already exist in the player's archive travel
as deltas against their own copy and are rebuilt here; only newly authored files travel
verbatim. Every reconstruction is hash-checked at both ends, and all of them happen before
anything is written, so a patch that cannot find its original aborts a pristine install
rather than leaving a half-applied one.

STORE COMPRESSION IS MANDATORY
------------------------------
The game's archive parser rejects any entry whose compression method is not 0, and .NET
Framework's ZipArchive cannot produce one -- CompressionLevel.NoCompression writes method 8
carrying stored blocks, which is both larger than the input and unreadable by the game. The
archive is therefore written by the stored-only writer in LhZip below, and the finished
file is re-opened and checked entry by entry before it replaces anything.

OUTPUT
------
Front ends set $script:LhLogSink to a scriptblock taking (message, level) where level is
one of Step/Ok/Warn/Bad/Info. Unset, everything goes to the host.
#>
...
# Output goes through one sink so a window can capture exactly what the console shows.
$script:LhLogSink = $null

function Write-Lh ($Message, $Level = 'Info') {
    if ($script:LhLogSink) { & $script:LhLogSink $Message $Level; return }
    switch ($Level) {
...
function Write-Step ($m) { Write-Lh $m 'Step' }
function Write-Ok   ($m) { Write-Lh $m 'Ok' }
function Write-Warn ($m) { Write-Lh $m 'Warn' }
function Write-Bad  ($m) { Write-Lh $m 'Bad' }
...
function Resolve-ModFolder ($Path) {
    <#
    A player has a .zip, not an unpacked folder, so accept either. Extraction goes to a
    short temp path on purpose: several resource paths run to ~110 characters and a deep
    destination pushes them past MAX_PATH, where the extractor drops files without saying
    so and the install fails later for a reason that looks nothing like the cause.
    #>
    $item = Get-Item -LiteralPath $Path
    if ($item.PSIsContainer) { return $item.FullName }
    if ($item.Extension -ne '.zip') { throw "Not a mod release: $Path" }

    $dest = Join-Path $env:TEMP ("lhmod-" + [System.IO.Path]::GetRandomFileName().Substring(0, 6))
    New-Item -ItemType Directory -Path $dest -Force | Out-Null
    [System.IO.Compression.ZipFile]::ExtractToDirectory($item.FullName, $dest)
    $inner = @(Get-ChildItem -LiteralPath $dest -Directory |
               Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName 'mod.json') })
    if ($inner.Count -eq 1) { return $inner[0].FullName }
    if (Test-Path -LiteralPath (Join-Path $dest 'mod.json')) { return $dest }
    throw "No mod.json inside $($item.Name) -- is it a Lionheart mod release?"
}


# --- setup ------------------------------------------------------------------
function New-ContextForId ($GameDirParam, $ModId) {
    # Uninstall needs no manifest: everything it restores was recorded in the game folder
    # at install time. The UI reaches this path, where the release the mod came from is
    # long gone.
    $game = Resolve-GameDir $GameDirParam
    $stateDir = Join-Path $game "mods\$ModId"
...
function Get-Context ($GameDirParam, $ModDirParam = $null) {
    if (-not $ModDirParam) { $ModDirParam = $ModDir }
    $manifestPath = Join-Path $ModDirParam 'mod.json'
    if (-not (Test-Path -LiteralPath $manifestPath)) {
        throw "No mod.json beside this script -- run it from inside the unzipped mod folder."
    }
    $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json

    $payloadPath = Join-Path $ModDirParam 'payload.json'
...
function Invoke-Install ($ctx) {
    $m = $ctx.Manifest
    Write-Lh "$($m.name) $($m.version)"
    Write-Lh "Installing to: $($ctx.Game)"
    ...
    foreach ($rel in $ctx.Payload.verbatim) {
        $srcPath = Join-Path (Join-Path $ctx.ModDir 'files') $rel
    ...
    try {
        foreach ($rel in $ctx.Payload.patched) {
            $patchPath = Join-Path (Join-Path $ctx.ModDir 'patches') "$rel.lhpatch"
    ...
    Write-Ok "Done. Start a NEW GAME -- see the readme for why."
}
...
function Invoke-Uninstall ($ctx, [switch]$Quiet) {
    ...
    if (-not $Quiet) {
        Write-Lh "Removing $($record.name) $($record.version) from $($ctx.Game)"
    }
    Assert-DiskSpace $ctx
